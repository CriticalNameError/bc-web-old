...
import {primary, primary_t80, primary_t50, primary_t60} from "helpers/colorScheme";
...
const ListBlogPosts = () => {
  ...
  const [first, setFirst] = useStateWithCallbackLazy(1);
  ...
  const [isLoadingMore, setIsLoadingMore] = useState(false);
        ...
        if (data) return (
           ...
           <Link to={"/blog/"+item.slug} style={{color: "#2c2c2c"}}>
                   <div class="card p-3 animated fadeInUp faster">

   

                       {item.postCategory.map((cat, index) => (
                           <span class="badge pull-right" style={{color: "white", backgroundColor: primary}}>{cat.title}</span>
                       ))}
                       <br></br>
                  
                   <div className={"row"}>
                     <div className={"col-md-6 col-12 text-center my-auto"}>
                      <img className={"img-raised img img-fluid img-blog"} src={item.titleImage.src}></img>
                     </div>
                        <div className={"col-12 col-md-6 text-center"}>
                        <i style={{fontSize: "0.8em"}}>{convertDateString(item.firstPublishedAt)}</i>
                        <h5 class="text-center mt-3 mt-md-0 mb-n1">{item.title}</h5>
                       
                       <center>
                        {item.tags.map((tag, index) => (
                           <span class="badge badge-secondary mx-1" style={{fontSize: "0.6em"}}>#{tag}</span>
                       ))}
                       </center>
                       <br></br>
          ...
          onClick={() => {
            ...
            fetchMore({
              ...
              updateQuery: (previousResult, { fetchMoreResult }) => {
                ...
                console.log([...previousResult.allPostPages, ...fetchMoreResult.allPostPages])
                return {
                 
                  allPostPages: [...previousResult.allPostPages, ...fetchMoreResult.allPostPages]
                }
